...
On January 24th 2025, the Hydra Doom tournament concluded in the finale happening at the [C-Play Summit event](https://lu.ma/cplay?tk=RqNYcG) at the HyperX Arena in Las Vegas. This was a live e-sports event where the finalists of previous `hydra-doom` stages were battling out a multiplayer deathmatch on-stage using our re-imagination of the 1993 ego-shooter Doom. See also this [hackernoon article](https://hackernoon.com/hydra-achieves-1-million-tps-validating-cardano-as-the-fastest-blockchain) for a high-level summary.

Let's look a bit under-the-hood of what we had deployed for this finale: Previous installments of the `hydra-doom` application were processing player inputs and games state updates as cardano transactions on a fleet of remotely hosted `hydra-node` processes and Hydra heads. This time, our fork of the doom [doom-wasm](https://github.com/cardano-scaling/doom-wasm) port was using Hydra to create a [`net_module_t`](https://github.com/cardano-scaling/doom-wasm/blob/9375223120772539e21165907cff8a1a9cf19bc3/src/net_hydra.c#L193-L196) that converts doom multiplayer network packets to cardano transactions and uses the Cardano ledger in a Hydra head to exchange them between multiple players. On top of this, the kills of each player were also recorded "on-chain" from similar transactions as used in the past. This allowed us to have a referee Hydra participant use the datum on-chain to update a "Series" smart contract and gather kill totals as a datum in a Cardano UTxO (on the L2).
